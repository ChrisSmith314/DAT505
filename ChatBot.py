...
Questions = ["How are you today //NAME//? ", "What have you been doing today? "]

Responses = [[["Good to hear."], ["That's a shame, at least I am having a good day."], ["What has been happening?"]], [["That sounds great."], ["sounds... interesting."], ["How has that been going?"]]]
...
def AskQuestions():
    ...
    print(QuestionNum)
    if(QuestionNum in AskedQuestions):
        AskQuestions()
    else:
        AskedQuestions.append(QuestionNum)
        if("//NAME//" in Questions[QuestionNum]):
            Question = Questions[QuestionNum].replace("//NAME//", Name)
        UserResponse = raw_input(Question)
        if(any(word in UserResponse for word in Positive)):
            if(random.randrange(1,2) == 2):
                print(Responses[QuestionNum][0][random.randrange(0, len(Responses[QuestionNum][0]))])
            else:
                print(Responses[QuestionNum][2][random.randrange(0, len(Responses[QuestionNum][2]))])
        elif(any(word in UserResponse for word in Negative)):
            print("You negative git")
        else:
            print("Here is just a random response")
